...
namespace Microsoft.BotBuilderSamples.Dialogs
{
    ...
    public class ActionDialog : CancelAndHelpDialog
    {
...

        {
            ...
            else if (entitiDetails.Intent == "Trigger_Service" || entitiDetails.Intent == "Build_Deployment" || entitiDetails.Intent == "TestDataGeneration")
            {
        ...
        private async Task<DialogTurnResult> PortfolioStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            switch (entitiDetails.Intent)
            {
              ...
              new PromptOptions
              {
                ...
                case "TestDataGeneration":
                    entitiDetails.Portfolio = ((FoundChoice)stepContext.Result).Value.ToString();
                    if (entitiDetails.Portfolio.Equals("CCV"))
                        return await stepContext.PromptAsync(nameof(TextPrompt), new PromptOptions { Prompt = MessageFactory.Text("Please enter DRG_CLASS for test data generation") }, cancellationToken);
                    else
                    {
        ...
        private async Task<DialogTurnResult> EnvironmentStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            switch (entitiDetails.Intent)
            {
                ...
                case "TestDataGeneration":
                    //drg class
                    entitiDetails.DbInstance = stepContext.Result.ToString().Trim();
                    return await stepContext.PromptAsync(nameof(TextPrompt), new PromptOptions { Prompt = MessageFactory.Text("Please enter payershort for test data generation") }, cancellationToken);
                default:
                    return await stepContext.NextAsync(entitiDetails.Project, cancellationToken);
        ...
        private async Task<DialogTurnResult> TagStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            else if (entitiDetails.Intent.Equals("TestDataGeneration"))
            {
                //payershort
                entitiDetails.Buildversion = stepContext.Result.ToString().Trim();
                return await stepContext.PromptAsync(nameof(TextPrompt), new PromptOptions { Prompt = MessageFactory.Text("Please enter oracle Id for test data generation") }, cancellationToken);
            }
            else
            {
        ...
        private async Task<DialogTurnResult> DbInstanceStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            switch (entitiDetails.Intent)
            {
                ...
                case "TestDataGeneration":
                    //oracle Id
                    entitiDetails.Buildwar = stepContext.Result.ToString().Trim();
                    return await stepContext.NextAsync(entitiDetails, cancellationToken);
                default:
                    return await stepContext.NextAsync(entitiDetails, cancellationToken);
            }
        ...
        private async Task<DialogTurnResult> ConfirmStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            switch (entitiDetails.Intent)
            {
                ...
                case "TestDataGeneration":
                    msg = $"Please confirm, Do you want test data generation with {entitiDetails.DbInstance} , {entitiDetails.Buildversion} , {entitiDetails.Buildwar} in {entitiDetails.Portfolio}?";
                    return await stepContext.PromptAsync(nameof(ConfirmPrompt), new PromptOptions { Prompt = MessageFactory.Text(msg) }, cancellationToken);
            }
        ...
        {  //var bookingDetails = await LuisHelper.ExecuteLuisQuery(Configuration, Logger, stepContext.Context, cancellationToken);
            ...
            else if (entitiDetails.Intent == "TestDataGeneration")
            {
                if (stepContext.Index > 6 || (bool)stepContext.Result)
                    return await stepContext.EndDialogAsync(entitiDetails, cancellationToken);
                else
        ...
        private IList<Choice> GetProjectChoices(string strPortFolio)
        {
                    ...
                    cardOptions = new List<Choice>()
            {
                ...
                new Choice() { Value = "Claim Status Tool", Synonyms = new List<string>() { "Claim Status Tool" } },
                new Choice() { Value = "Claim Status Tool API", Synonyms = new List<string>() { "Claim Status Tool API" } },
        ...
        IList<Choice> GetAnnocoderChoice(string strType)
        {
            switch (strType)
            {
                ...
                case "Client":
                    ...
                    return cardOptions;
                case "CPW":
                case "PresentationManager":
                case "CPD_SwitchControl":
                    cardOptions = new List<Choice>() {
                    new Choice() { Value = "DEV", Synonyms = new List<string>() { "DEV" } },
                    new Choice() { Value = "QA1", Synonyms = new List<string>() { "QA1" } },
                    new Choice() { Value = "QA2", Synonyms = new List<string>() { "QA2" } },
                    new Choice() { Value = "UAT", Synonyms = new List<string>() { "UAT" } },
                    };
                    return cardOptions;
                default:
                    ...
                    new Choice() { Value = "QA", Synonyms = new List<string>() { "QA" } },
                    new Choice() { Value = "UAT", Synonyms = new List<string>() { "UAT" } },
                    new Choice() { Value = "SPRINTTEST", Synonyms = new List<string>() { "SPRINTTEST" } },
                    new Choice() { Value = "SPRINTDEMO", Synonyms = new List<string>() { "SPRINTDEMO" } },
                    };
                    return cardOptions;
